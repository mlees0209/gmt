...
GMT_LOCAL char gmtinit_find_argument (struct GMTAPI_CTRL *API, char *longlist, char *shortlist, char *textin, int sepcode, char *argout) {
	...
	if (m && c)			/* Found both short-option flag and an argument following sepcode */
		strcpy (argout, &c[1]);	/* Pass back out the directive/modifier argument */
	else if (m)			/* Found a short-option flag but no arguments follow */
		argout[0] = '\0';	/* No sepcode found, i.e., no post-directive/-modifier argument */
	else				/* No directive/modifier, m is 0 and c is NULL */
		strcpy (argout, textin);/* Not a directive/modifier, pass out the argument as is */
	switch (sepcode) {		/* restore sepcode within textin as appropriate */
	case GMT_FINDARG_COLON:
		if (c) c[0] = ':';
		break;
	}
